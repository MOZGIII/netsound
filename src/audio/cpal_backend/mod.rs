...
fn build(builder: super::BackendBuilder) -> Result<Backend, errors::Error> {
...
impl super::Backend for Backend {
    fn run(&mut self) {
        let shared_input_buf = &mut self.input_buf;
        let shared_output_buf = &mut self.output_buf;
        self.cpal_eventloop.run(move |_, data| {
            match data {
                cpal::StreamData::Input { buffer: input_buf } => {
                    let mut self_input_buf = &mut *shared_input_buf.lock();
                    assert!(self_input_buf.len() + input_buf.len() <= self_input_buf.capacity());
